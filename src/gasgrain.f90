...
!! HOOK: the pointer is kept so that a time-dependent 0D structure (collapsing
!! core, parcel trajectory, ...) can be added later as an extra prescription
...
CHASUM=0.d0
do I=1,nb_species
    if (I.NE.INDEL) CHASUM=CHASUM+SPECIES_CHARGE(I)*abundances(I)
enddo
if (CHASUM.LE.0.d0) CHASUM=MINIMUM_INITIAL_ABUNDANCE
abundances(INDEL)=CHASUM


! Set the electron abundance via conservation===========
! And check at the same time that the initial abundances have the same elemental
! abundances as the control file.
call check_conservation(abundances(1:nb_species))

! Physical properties of the 0D box
