...
!   enddo
  ...
  !-------------------------------------------------------------
  !  (10-11) Formation of H2 on grains when is_grain_reactions = 0.
  !
  !  Comments:
  !     - In the case is_grain_reactions = 0 we still need the formation
  !     of H on the grains; this is done with the XH species
  !     through those reaction types.
  !
  !     For these reactions we do not use the grain size distribution:
  !     a single grain size is used, the radius given in parameters.in.
  !
  !  NOTE: the 'is_h2_adhoc_form' branch of nmgc-2.0 has been removed.
  !-------------------------------------------------------------
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
  IF (is_h2_formation_rate.eq.1) THEN ! h2_formation flag in parameters.in
      do J=type_id_start(97),type_id_stop(97)
              stick_Tgas = 1/(1 + (gas_temperature/T_2)**beta) ! sticking function at gas_temperature
              stick_100 = 1/(1 + (100./T_2)**beta) ! reference sticking function at 100K. Bron et al. (2014) used 100K.
              n_eq = H_number_density*sqrt(gas_temperature/100)*(stick_Tgas/stick_100) ! equivalent density of H.
